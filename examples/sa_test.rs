...
use mozjpeg_oxide::progressive::{generate_mozjpeg_max_compression_scans, generate_minimal_progressive_scans};
use std::fs::File;
use std::path::Path;
...
fn main() {
    ...
    for (i, scan) in generate_minimal_progressive_scans(3).iter().enumerate() {
        println!("Scan {}: Ss={:2} Se={:2} Ah={} Al={}", i, scan.ss, scan.se, scan.ah, scan.al);
    }
    ...
    for quality in [50, 75, 85, 90, 95, 97] {
        ...
        println!(
            ...
            quality, simple.len(), simple_ok, c_jpeg.len(), c_ok, diff
        );
    }
}
