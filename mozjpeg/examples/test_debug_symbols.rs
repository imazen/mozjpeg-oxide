...
use mozjpeg::huffman::FrequencyCounter;
...
fn main() {
    ...
    // Decode baseline
    let base_dec = jpeg_decoder::Decoder::new(std::io::Cursor::new(&baseline)).decode().unwrap();
    ...
    match progressive {
        Ok(data) => {
            ...
            match jpeg_decoder::Decoder::new(std::io::Cursor::new(&data)).decode() {
                Ok(decoded) => {
                    let mse: f64 = rgb_data.iter()
                        .zip(decoded.iter())
                        .map(|(&a, &b)| {
                            let diff = a as f64 - b as f64;
                            diff * diff
                        })
                        .sum::<f64>() / rgb_data.len() as f64;
    ...
    match freq.generate_table() {
        Ok(table) => {
            println!("Generated table with {} symbols", table.bits.iter().sum::<u8>());
        }
        Err(e) => {
