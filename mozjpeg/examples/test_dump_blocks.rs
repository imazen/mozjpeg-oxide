...
fn main() {
    ...
    println!("Saved baseline ({} bytes) and progressive ({} bytes)",
        baseline.len(), progressive.len());
    ...
    let output = std::process::Command::new("djpeg")
        .args(["-pnm", "-outfile", "/tmp/dump_base.ppm", "/tmp/dump_baseline.jpg"])
        .output();
    match output {
        Ok(o) => {
            if !o.status.success() {
                println!("djpeg baseline failed: {}", String::from_utf8_lossy(&o.stderr));
            } else {
    ...
    let output = std::process::Command::new("djpeg")
        .args(["-pnm", "-outfile", "/tmp/dump_prog.ppm", "/tmp/dump_progressive.jpg"])
        .output();
    match output {
        Ok(o) => {
            if !o.status.success() {
                println!("djpeg progressive failed: {}", String::from_utf8_lossy(&o.stderr));
            } else {
    ...
    if let (Ok(base_ppm), Ok(prog_ppm)) = (
        ...
        if let (Some(base_pixels), Some(prog_pixels)) = (parse_ppm(&base_ppm), parse_ppm(&prog_ppm)) {
            ...
            for i in 0..8 {
                ...
                let base = (base_pixels.get(i*3).copied().unwrap_or(0),
                           base_pixels.get(i*3+1).copied().unwrap_or(0),
                           base_pixels.get(i*3+2).copied().unwrap_or(0));
                let prog = (prog_pixels.get(i*3).copied().unwrap_or(0),
    ...
    fn count_sos_markers(data: &[u8]) -> Vec<(usize, u8, u8)> {
        ...
        while i + 1 < data.len() {
            if data[i] == 0xFF && data[i+1] == 0xDA {
                ...
                let ss = if i + length < data.len() { data[i + length - 1] } else { 0 };
                let se = if i + length + 1 < data.len() { data[i + length] } else { 0 };
