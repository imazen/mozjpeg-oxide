...
//! ## Zigzag Traversal
//!
//! The algorithm processes pixels in "natural order" (zigzag), treating the 8x8 block
//! as a 1D sequence. This is the same order used for DCT coefficient encoding and
//! provides a reasonable approximation of spatial adjacency.
//!
//! ## Catmull-Rom Spline
//!
//! The interpolation uses Catmull-Rom splines, which pass through all control points
//! and provide smooth C1 continuity. The formula:
//!
//! ```text
//! P(t) = 0.5 * [(2*P1) + (-P0 + P2)*t + (2*P0 - 5*P1 + 4*P2 - P3)*t² + (-P0 + 3*P1 - 3*P2 + P3)*t³]
//! ```
//!
//! For overshoot, we use a variant that allows specifying tangent slopes at the endpoints.
//!
//! ## Overshoot Limit
//!
...
//!
//! ```text
//! maxovershoot = maxsample + min(
//!     min(31, 2 * DC_quant_value),        // Don't overshoot too much
//!     (maxsample * 64 - sum) / count      // Stay within DC headroom
//! )
//! ```
//!
//! This ensures the DC coefficient doesn't overflow and limits the additional bits needed.
//!
//! # References
//!
...
use crate::consts::{DCTSIZE, DCTSIZE2, JPEG_NATURAL_ORDER};
...
///
/// # Arguments
/// * `value1` - Point before the interpolation segment (for tangent calculation)
/// * `value2` - Start point of interpolation
/// * `value3` - End point of interpolation
/// * `value4` - Point after the interpolation segment (for tangent calculation)
/// * `t` - Interpolation parameter (0.0 = value2, 1.0 = value3)
/// * `size` - Length of the segment (scales tangents)
///
/// # Returns
/// Interpolated value at position `t` between value2 and value3.
...
///                       ↑    ↑    ↑
///                  These overshoot 127 but will clamp to 255 when decoded
/// ```
...
mod tests {
    ...
    fn test_deringing_smooth_curve() {
        ...
        data[JPEG_NATURAL_ORDER[0]] = 20;
        data[JPEG_NATURAL_ORDER[1]] = 60;
        data[JPEG_NATURAL_ORDER[2]] = 100;
        for i in 3..8 {
            data[JPEG_NATURAL_ORDER[i]] = MAX_SAMPLE;
        }
        // Then ramp down
        data[JPEG_NATURAL_ORDER[8]] = 100;
        data[JPEG_NATURAL_ORDER[9]] = 60;
        data[JPEG_NATURAL_ORDER[10]] = 20;

        preprocess_deringing(&mut data, 16);

        // The max-value region should now have a curve
        // The middle values should be higher than the edges of the curve
        let edge_val = data[JPEG_NATURAL_ORDER[3]];
        let mid_val = data[JPEG_NATURAL_ORDER[5]];
